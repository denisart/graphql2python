...
def test_schema_validation_raises(input_file: str):
    with pytest.raises(ValueError):
        GraphQL2PythonModelConfig(schema=input_file, output=static_folder / "output_file.py")


def test_schema_validation_correct():
    config = GraphQL2PythonModelConfig(
        schema=Path(static_folder / "example.graphql"), output=static_folder / "output_file.py"
    )
